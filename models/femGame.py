...
class FemGame(db.Model):
    id = db.Column(mysql.INTEGER(50), primary_key=True)
    republic_home_id = db.Column(db.Integer, db.ForeignKey('fem_republic.id'),nullable=False)
    republic_away_id = db.Column(db.Integer, db.ForeignKey('fem_republic.id'),nullable=False)
    republic_home = db.relationship(FemRepublic, foreign_keys=[republic_home_id])
    republic_away = db.relationship(FemRepublic, foreign_keys=[republic_away_id])
